	...
	*/
	...
	protected function get_link_list_fields($standard_class = true)
	{
		...
		foreach($this->woocommerce_additional_fees->gateways as $k => &$gateway)
		{
			...
			if($s != $gateway->title) $s.= ' ('.$gateway->title.')';
	...
	public function get_gateway_inputfields($key, $gateway)
	{
		...
		$default = $this->options[woocommerce_additional_fees::OPT_GATEWAY_PREFIX][$key][woocommerce_additional_fees::OPT_KEY_VALUE_TO_ADD_FIXED];
		$name = $this->create_unique_html_name($key, woocommerce_additional_fees::OPT_KEY_VALUE_TO_ADD_FIXED);
		$this->fields[] = array(
			'type' => 'number',
			'custom_attributes' => array('step'=>'any'),
			'id' => $name,
			'title' => __('Fixed Value to add:', woocommerce_additional_fees::TEXT_DOMAIN),
			'default' => $default,
			'desc' => __('Enter a fixed value to be added in addition to the calculated fees above (e.g. a fixed transaction fee charged by the credit card company). Enter the value according to your settings in \'Prices entered with/without tax\'.', woocommerce_additional_fees::TEXT_DOMAIN),
			'desc_tip' => true
			);
	...
	public function save_options_settings()
	{
		...
		foreach($this->woocommerce_additional_fees->gateways as $key => &$gateway)
		{
			...
			if($this->check_numeric($value, $name))
			{
				$this->options[woocommerce_additional_fees::OPT_GATEWAY_PREFIX][$key][woocommerce_additional_fees::OPT_KEY_VALUE_TO_ADD_FIXED] = $value;
			}
