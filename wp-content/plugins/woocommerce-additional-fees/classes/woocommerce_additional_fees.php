...
class woocommerce_additional_fees
{
	...
	/**
	 * set to true, if SPL autoregister is not present - a fallback situation only
	 * 
	 * @var boolean
	 */
	static public $no_autoload;
	...
	public function __construct()
	{
		...
		add_action('woocommerce_init', array($this, 'handler_wc_init'), 10);
	...
	static public function autoload($class_name)
	{
		...
		switch ($class_name)
		{
			case 'wc_calc_add_fee':
			case 'wc_order_addfee':
				require_once self::$plugin_path.'classes/'.$class_name.'.php';
				break;
			default:
				break;
		}
	...
	public function handler_wc_init()
	{
		if(self::$no_autoload)
		{
			if(!class_exists('wc_order_addfee'))
			{
				require_once self::$plugin_path.'classes/wc_order_addfee.php';
			}
		}
		$this->init_values();
	}
	...
	protected function attach_to_hooks()
	{
		 ...
		 */
		add_action('woocommerce_cart_calculate_fees', array($this, 'handler_wc_cart_calculate_fees'), 10, 1);
		 ...
		 */
		add_action('woocommerce_resume_order', array($this, 'handler_wc_resume_order'), 10, 1);
		 ...
		 */
		add_action('woocommerce_add_order_fee_meta', array($this, 'handler_wc_add_order_fee_meta'), 10, 4);
	}
	...
	static public function &get_post_meta_product_default($post_id)
	{
		...
		$default = array(
			woocommerce_additional_fees::OPT_ENABLE_PROD => 'yes',
			woocommerce_additional_fees::OPT_GATEWAY_PREFIX => array()
			);
	...
	{
		$default = array(
			woocommerce_additional_fees::OPT_ENABLE_RECALC => 'yes',
			woocommerce_additional_fees::OPT_ENABLE_RECALC_SAVE_ORDER => 'yes',
			woocommerce_additional_fees::OPT_FIXED_GATEWAY => 'no',
			woocommerce_additional_fees::OPT_KEY_FEE_ITEMS => array()
			);
	...
	static public function &get_option_gateway_default(array $option_gateway, $gateway_name = '', $for_postmeta = false)
	{
		...
		$default = array(
			...
			woocommerce_additional_fees::OPT_KEY_VALUE_TO_ADD_FIXED => 0,
	...
	public function handler_wc_cart_calculate_fees(WC_Cart $obj_wc_cart)
	{
		...
		if ( sizeof( $obj_wc_cart->cart_contents ) > 0 )
		{
			foreach ($obj_wc_cart->cart_contents as $cart_item_key => $values )
			{
				...
				$fees_calc = $this->calculate_gateway_fee_product($_product, $obj_wc_cart->tax, $obj_wc_cart->prices_include_tax, $total_excl, $total_incl, $values['quantity']);
	...
	public function handler_wc_calculate_totals($total)
	{
		...
		$total_incl_tax = max( 0, round( $obj_wc_cart->cart_contents_total + $obj_wc_cart->tax_total + $obj_wc_cart->shipping_tax_total + $obj_wc_cart->shipping_total - $obj_wc_cart->discount_total + $obj_wc_cart->fee_total, $obj_wc_cart->dp ) );
		...
		$fee_total = $this->calculate_gateway_fee_total ($obj_wc_cart->tax, $obj_wc_cart->prices_include_tax, $total_excl_tax, $total_incl_tax);
		if(! isset($fee_total))
		{
			return $total;
		}
	...
	public function handler_wc_before_pay()
	{
		...
		if(empty( $wp->query_vars['order-pay'])) return;
		...
		$info .= 'add_fee_fixed_gateway="'.$pm[woocommerce_additional_fees::OPT_FIXED_GATEWAY].'" ';
	...
	public function handler_wc_add_order_fee_meta($order_id, $item_id, $fee, $fee_key)
	{
		...
		if(empty($fee->data_source)) return;
		if(! $fee->data_source instanceof wc_calc_add_fee) return;
		if($fee->data_source->source != self::OPTIONNAME) return;
	...
	public function handler_ajax_wc_remove_order_item()
	{
		...
		if(! isset($_POST['order_item_ids'])) return;
		
		$order_item_ids = $_POST['order_item_ids'];
		
		if(!is_array($order_item_ids)) $order_item_ids = array($order_item_ids);
		
		if ( sizeof( $order_item_ids ) == 0 ) return;
	...
	public function handler_ajax_calc_fee_pay_order()
	{
		...
		$pm = self::get_post_meta_order_default($order_id);
		if($pm[woocommerce_additional_fees::OPT_ENABLE_RECALC] != 'yes')
		{
			$response ['success'] = true;
			$response ['recalc'] = false;
			echo json_encode( $response );
		...
		if(!$this->request_data_loaded)	$this->load_request_data($add_fee_new_paymethod);
	 ...
	 */
	protected function &calculate_fees(WC_Tax $obj_wc_tax, $includes_tax, $value, array $gateway, $quantity = 1)
	{
			//	get tax rates
...
//				break;
	...
	protected function add_fee_to_cart(wc_calc_add_fee &$fee, WC_Cart $obj_wc_cart)
	{
		...
		$taxable = $fee->taxable;
		$tax_class = $tax_class = $fee->gateway_option[woocommerce_additional_fees::OPT_KEY_TAXCLASS] == self::VAL_TAX_STANDARD ?  '' : $fee->gateway_option[woocommerce_additional_fees::OPT_KEY_TAXCLASS];
...
//		}
	 ...
	 * @return wc_calc_add_fee
	 */
	protected function &calculate_gateway_fee_total (WC_Tax &$obj_wc_tax, $includes_tax, $total_excl, $total_incl)
	{
		$fees_calc = null;
		...
		$fees_calc = $this->calculate_fees($obj_wc_tax, $includes_tax, $total_incl, $this->payment_gateway_option);
	 ...
	 * @param float $total_incl
	 * @param int $quantity
	 * @return wc_calc_add_fee     null, if no fee to add, else wc_calc_add_fee
	 */
	protected function &calculate_gateway_fee_product(WC_Product $_product, WC_Tax &$obj_wc_tax, $includes_tax, $total_excl, $total_incl, $quantity = 1)
	{
		$fees_calc = null;
		...
		if($gateway[self::OPT_KEY_ENABLE] != 'yes') return $fees_calc;
		...
		//changed with 2.1.0 - replaced $total_excl with $total_incl
		$fees_calc = $this->calculate_fees($obj_wc_tax, $includes_tax, $total_incl, $gateway, $quantity);

		$fees_calc->source = self::OPTIONNAME;
		$fees_calc->type = wc_calc_add_fee::VAL_PRODUCT_ADD_FEE;
		$fees_calc->id = substr(('ADD_FEE_'.$_product->id), 0, 15);
		$fees_calc->gateway_key = $this->payment_gateway_key;
		$fees_calc->gateway_title = $this->gateways[$this->payment_gateway_key]->title;
